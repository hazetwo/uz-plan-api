...
@router.get("/")
async def get_entries(url: str, client: HttpClient) -> List[ScheduleEntry]:
...
@router.get("by-day")
async def get_entry(url: str, date: date, client: HttpClient):
    soup = await fetch_schedule(url, client)
...
async def get_week_entries(
    url: str, date: date, client: HttpClient
) -> List[ScheduleEntry]:
    soup = await fetch_schedule(url, client)
    schedule_entries = parse_schedule(soup)
